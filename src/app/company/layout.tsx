...
export default function CompanyLayout({ children }: { children: React.ReactNode }) {
  ...
  // Check manual expiry if status is active but date passed
  const now = new Date();
  const expiryDate = company?.expiryDate ? new Date(company.expiryDate) : null;
  const isDateExpired = expiryDate && now > expiryDate;

  if (isExpired || isSuspended || isDateExpired) {
    return (
      <div className="h-screen w-full fixed inset-0 z-[9999] bg-background/95 backdrop-blur-md flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-[40px] shadow-2xl p-12 text-center space-y-8 border-4 border-destructive/20 animate-in zoom-in-95">
          ...
          <div className="space-y-2">
            <h2 className="text-3xl font-black tracking-tighter">{company?.name} Dashboard Already Expired</h2>
          ...
          <div className="p-6 bg-secondary/20 rounded-2xl">
             ...
             <p className="text-xl font-black text-destructive uppercase">{isDateExpired ? 'EXPIRED' : company?.status}</p>
             {expiryDate && <p className="text-[10px] font-bold text-muted-foreground mt-1">Lapsed on: {expiryDate.toLocaleDateString()}</p>}
          </div>
